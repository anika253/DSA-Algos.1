...
// If TimeUtils is not a class/namespace, define getCurrentTime() here for demonstration
inline std::string getCurrentTime() {
    // Replace with actual time retrieval logic as needed
...
public:
    Order* createOrder(User* user, Cart* cart, Restaurant* restaurant, const vector<MenuItem>& menuItems,
        ...
        order->setScheduled(getCurrentTime());
        }
        order->setUser(user);
